...
var fs = {
	...
	//	log('copyFile - ', src, dst);
		...
		try{
			...
			var to   = filesystem.createWriteStream(dst, {flag: 'w+'});
			from.on('open', function() {
				// log( "OPEN READ STREAM");
				from.pipe( to );
			});
			// log( from, to );
			// .on('readable',  function () { console.log('readable!');  })
		    // to.on('data',  function (data) { console.log('Data!', data);  })
		    to.on('error', function (err)  { console.error('Error', err);/* READ if there was an error receiving data. */ })
